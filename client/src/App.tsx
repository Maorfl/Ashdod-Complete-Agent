...
// איפוס גלילה במעבר עמוד — React Router לא מאפס לבד, ומעבר לעמוד קצר מרגיש כ"קפיצה"
...
function Shell() {
    ...
    useEffect(() => {
        api.version()
            .then((v) => setVer(v))
            .catch(() => setVer({ current: "1.0.0", update_required: false }));
    }, []);
        ...
        <div className="shell">
            ...
            <nav className="nav">
                ...
                <NavLink to="/" className={nav} end>
                    <span className="ic">▤</span> דשבורד מטענים
                </NavLink>
                <NavLink to="/approvals" className={nav}>
                    <span className="ic">✆</span> אישורי שליחה
                </NavLink>
